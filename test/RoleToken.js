...
contract('RoleToken', function(accounts) {
    ...
    it('initialized the contract with the correct values', function() {
      return RoleToken.deployed().then(function(instance) {
        ...
        assert.equal(name, 'Role Token', 'has the correct name');
        return tokenInstance.symbol();
      }).then(function(symbol) {
        assert.equal(symbol, 'RT', 'has the correct symbol');
      });
    });
    ...
    it('transfers token ownership', function() {
      return RoleToken.deployed().then(function(instance) {
        ...
        assert.equal(balance.toNumber(), 99750, 'deducts the amount from the sending account');
      });
    })
});
